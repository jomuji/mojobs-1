...
?>
    ...
    <div class="wpjb-top-header wpjb-layer-inside">
        <div class="wpjb-top-header-image col-sm-3">
            
            <?php if($job->doScheme("company_logo")): ?>
            <?php elseif($job->getLogoUrl()): ?>
            <img src="<?php echo $job->getLogoUrl("300x200") ?>" alt="" class="img-responsive" />
            <?php else: ?>
            <span class="wpjb-glyphs wpjb-icon-building wpjb-icon-only wpjb-icon-64"></span>
            <?php endif; ?>
        </div>
            
        <div class="wpjb-top-header-content col-sm-8">
            <div>
                ...
                <em class="wpjb-top-header-subtitle">
                    <?php echo sprintf(__('Published: %1$s', "wpjobboard"), wpjb_date_display(get_option('date_format'), $job->job_created_at), daq_time_ago_in_words($job->time->job_created_at)) ?>
                </em>
