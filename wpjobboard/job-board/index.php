...
<div class="wpjb wpjb-page-index">
    ...
    <div id="wpjb-top-search" class="wpjb-layer-inside">
        <form action="<?php esc_attr_e(get_permalink($page_id)) ?>" method="GET" id="wpjb-top-search-form">
        <?php global $wp_rewrite ?>
        <?php if(!$wp_rewrite->using_permalinks()): ?>
        <input type="hidden" name="page_id" value="<?php echo $page_id ?>" />
        <input type="hidden" name="job_board" value="find" />
        <?php endif; ?>
        <input autocomplete="off" type="text" class="wpjb-top-search-query wpjb-ls-query" name="query" value="<?php esc_attr_e(isset($param["query"]) ? $param["query"] : "") ?>" placeholder="<?php esc_attr_e("Keyword ...", "wpjobboard") ?>" />
        <input autocomplete="off" type="text" class="wpjb-top-search-location wpjb-ls-location" name="location" value="<?php esc_attr_e(isset($param["location"]) ? $param["location"] : "") ?>" placeholder="<?php esc_attr_e("Location ...", "wpjobboard") ?>" />
        <br/>
        <ul>
